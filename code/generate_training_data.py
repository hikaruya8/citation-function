...
def main():
    ...
    with open('feature-counts.tsv', 'w') as feat_out:
        for (pattern, feat, clazz), count in list(FEATURE_FIRING_COUNTS.items()):
    ...
    for paper_id, paper_feature_dicts in list(paper_to_feature_dicts.items()):
        for (labels, paper_feature_dict) in paper_feature_dicts:
            for feature, val in list(paper_feature_dict.items()):
    ...
    for feat, vals in list(feature_to_all_vals.items()):
        vals.sort()
...
def get_paper_features(json_fname):
    ...
    for cited_id, vecs in list(cited_id_to_vecs.items()):
        ...
        for sec, count in list(cites_per_section.items()):
        ...
        for sec, count in list(ind_cites_per_sec.items()):
